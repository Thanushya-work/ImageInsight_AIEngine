...
#
# Notes:
#   - 'others' and 'offe' (typo class) are intentionally EXCLUDED — unmapped
#   - 'flane' (typo duplicate of 'flange') maps to the same ID as 'flange'
#   - 'well_branding' (typo duplicate of 'wall_branding') maps to same ID
...
# ---------------------------------------------------------------------------
UNMAPPED_YOLO_NAMES = {
    "others",   # index 21 — generic catch-all → triggers GPT fallback
}

SILENTLY_IGNORED = {
    "offe",     # index 42 — truncated typo class → discard, no GPT fallback
}
...
def run_yolo_analysis(
    ...
    classtext_cache = {}
    for cid in range(1018, 1098):   # extended to cover new class IDs up to 1097
