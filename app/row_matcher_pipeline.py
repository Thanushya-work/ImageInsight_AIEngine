...
def compute_counts(mapped_caps, sku_detections):
    ...
    Case B  total_caps >= CAP_COUNT_THRESHOLD
        ...
        Three signals are computed; the highest wins:
    ...
    for sku in sku_groups:
        ...
        columns = round(shelf_width / avg_w) if avg_w > 0 else 1
        ...
        # ------------------------------------------------------------------
        # Final depth — highest signal wins, clamped to [1, 8]
        # ------------------------------------------------------------------
        depth = round(max(depth_caps, perspective_depth, overlap_depth))
