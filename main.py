...
def execute_models(pod_id, iterationid, stagingid):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # ── YOLO analysis ─────────────────────────────────────────────────
            logger.info("Starting YOLO activation analysis")
            try:
                output_csv = ollama_config.get('output_csv', 'outputs/yolo_analysis_results.csv')
                ...
                yolo_results, yolo_csv = run_yolo_analysis(
                    ...
                    s3_handler=s3_handler,
                    s3_annotated_folder=(
                        f"{ollama_config['output_s3_folder']}VisibleItem_{cyclecountid}"
                    ),
                    db_config=db_config,
                    cyclecountid=cyclecountid,
                    stagingid=stagingid
                ...
                logger.info(
                    f"YOLO analysis complete: "
                    f"{len(yolo_results) if yolo_results else 0} records"
                )
            except Exception as e:
                logger.error(f"YOLO analysis failed: {e}")
                logger.error(traceback.format_exc())
                yolo_results, yolo_csv = [], None

            # ── Upload analysis CSV to S3 ─────────────────────────────────────
            if yolo_csv and os.path.exists(yolo_csv):
                logger.info("Uploading YOLO CSV to S3")
                try:
                    s3_handler.upload_file_to_s3(
                        yolo_csv,
                        f"{ollama_config['output_s3_folder']}VisibleItem_{cyclecountid}/yolo_analysis_results.csv"
                    )
                    logger.info("YOLO CSV uploaded successfully")
                except Exception as e:
...
def main():
    ...
    # Get stagingid once — all batches share this value
    ...
    try:
        ...
        # Clear structural count temp table (created by visicooler.py for
        # cap_pipeline_runner Step 5; may not exist on very first run).
        # NOTE: psycopg2 cannot bind parameters inside a DO $$ ... $$ anonymous
        # block — Postgres treats the block as a plain string with zero bind
        # slots, so passing a parameter dict raises
        # "bind message supplies 1 parameters, but prepared statement requires 0".
        # We guard existence with a plain SELECT first, then DELETE separately.
        cur.execute("""
            SELECT 1
            FROM information_schema.tables
            WHERE table_schema = 'temp'
              AND table_name   = 'structural_count_temp'
        """)
