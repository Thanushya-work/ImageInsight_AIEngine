...
class PipelineResult:
    ...
    def log_summary(self) -> None:
        sep = "=" * 100
        logger.info(sep)
        logger.info("  CAP PREDICTION PIPELINE  –  SUMMARY (4 steps)")
        logger.info(sep)
        logger.info(f"  Iteration ID   : {self.iteration_id}")
        logger.info(f"  Overall status : {self.overall_status.upper()}")
        logger.info(f"  Total duration : {self.total_duration_ms:.1f} ms")
...
def _build_queries(iteration_id: int) -> list[dict]:
    ...
    Old Step 2 – "Insert CAP predictions from SKU table"
        ...
        created ghost rows: the 5 front-facing bottles that were correctly
        mapped in Python were re-inserted as bare SKU rows with
        prod_class_id = NULL, doubling the count for every front-facing SKU.

    Old Steps 3 & 4 – SQL vertical-match / centroid-distance remapping
        These re-mapped prod_class_id in SQL on top of the Python mapping,
        effectively doing the work twice.  Because the Python mapper is
        brand+package-aware and IoU-deduped it produces better results;
        the SQL fallback is no longer needed.

    Old Step 5 – "Remove small caps inside SKU bounding box (< 15 % area)"
        With Steps 2-4 gone, cap rows in the table are *only* cap-model
        detections; they will never spatially coincide with the SKU bounding
        boxes stored in sku_prediction_temp at ≥85 % scale (caps are always
        smaller than the bottle body).  The Python IoU-dedup in cap_sku_mapper
        already handles overlapping duplicate caps before insertion.
        Keeping this DELETE therefore removes legitimate detections in edge
        cases where a cap happens to sit inside a slightly larger SKU box
        (e.g. tight-crop annotated images).

    Remaining pipeline (renumbered 1-5):
        ...
        4  Populate coolermetricstransaction + update caserid
    """
    iid = int(iteration_id)          # guard against injection
    return [
        # ── 1 ─────────────────────────────────────────────────────────────────
        ...
        # ── 2 ─────────────────────────────────────────────────────────────────
        # NOTE: Steps 2-5 from the old pipeline (SKU→cap insert, SQL vertical
        # match, centroid fallback, small-cap delete) have been removed.
        # cap_sku_mapper.py already handles all cap→SKU mapping in Python
        # before insertion, so repeating it here caused duplicates.
        # ──────────────────────────────────────────────────────────────────────
        {
            "step": 2,
            "name": "Remove duplicate rows",
...
def run_cap_pipeline(db_config: dict, iteration_id: int) -> PipelineResult:
    """
    Execute all 4 CAP post-processing steps sequentially.
